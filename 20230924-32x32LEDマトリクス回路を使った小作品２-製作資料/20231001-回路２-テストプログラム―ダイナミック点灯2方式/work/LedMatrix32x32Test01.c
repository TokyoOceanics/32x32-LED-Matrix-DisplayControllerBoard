/*
 * LedMatrix32x32Test01.c (横溝先生のled_matrix_32_32_k7_k1.cの完全コピー）
 *
...
int main(void)
{
  ...
  SysTick_Config(RCC_Clocks.HCLK_Frequency/30000);
...
void SysTick_Handler(void){
    ...
    //出力イネーブル
	//if((clk==0)&&(clk_count==23)){
    if(clk_count==23){
	      GPIOB->ODR |= OE_HI;      //OEB=1
	}
	//if((clk==0)&&(clk_count==20)){
    if(clk_count==20){
